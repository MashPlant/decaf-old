...
impl LLVMCodeGen {
  ...
  unsafe fn cur_bb_unterminated(&self) -> bool {
    LLVMGetBasicBlockTerminator(LLVMGetLastBasicBlock(self.cur_fn)).is_null()
  }
  ...
  unsafe fn new_bb(&self) -> LLVMBasicBlockRef {
    LLVMAppendBasicBlockInContext(self.context, self.cur_fn, T)
  }

  // this is to adjust the order of bb
  // so that the logically last-added bb will be at last
  unsafe fn move_bb_to_end(&self, bb: LLVMBasicBlockRef) {
    LLVMMoveBasicBlockAfter(bb, LLVMGetLastBasicBlock(self.cur_fn));
  }
  ...
  unsafe fn method(&mut self, method: &mut MethodDef) {
    ...
    if LLVMGetBasicBlockTerminator(LLVMGetLastBasicBlock(self.cur_fn)).is_null() {
  ...
  unsafe fn expr(&mut self, expr: &mut Expr) {
    ...
    expr.llvm_val = match &mut expr.data {
      ...
      Indexed(indexed) => {
        ...
        LLVMPositionBuilderAtEnd(builder, on_err);
      ...
      Binary(binary) => {
        ...
        match binary.op {
          ...
          And | Or => {
            ...
            LLVMPositionBuilderAtEnd(builder, eval_r);
            self.expr(&mut binary.r);
